...
class _HistoryScreenState extends ConsumerState<HistoryScreen> {
  ...
  List<_HistoryListRow> _buildRows(List<_HistorySection> sections) {
    final rows = <_HistoryListRow>[];
    for (final section in sections) {
      rows.add(_HistoryLabelRow(section.label));
      for (final item in section.items) {
        rows.add(_HistoryItemRow(item));
      }
    }
    return rows;
  }
  ...
  void _resetFilters(WidgetRef ref) {
    ...
    ref.read(historyPlatformFilterProvider.notifier).clear();
    ref.read(historySortProvider.notifier).reset();
  }
                  ...
                  children: [
                    Padding(
                      ...
                      child: _HistoryControls(
                        ...
                        onSortChanged: (sort) {
                          ref.read(historySortProvider.notifier).set(sort);
                        },
                        onResetFilters: () {
                          _resetFilters(ref);
                        },
                          ...
                          : RefreshIndicator(
                              ...
                              child: ListView.builder(
                                ...
                                itemBuilder: (buildContext, index) {
                                  ...
                                  if (row is _HistoryLabelRow) {
                                    return _DateHeader(label: row.label);
                                  }

                                  final historyItemRow = row as _HistoryItemRow;
                                  return _HistoryCard(
                                    item: historyItemRow.item,
                                    locale: locale,
                                    onTap: () {
                                      unawaited(
                                        _openChat(
...
class _HistoryLabelRow extends _HistoryListRow {
  final String label;

  const _HistoryLabelRow(this.label);
}

class _HistoryItemRow extends _HistoryListRow {
  final ChatHistoryItem item;

  const _HistoryItemRow(this.item);
}
